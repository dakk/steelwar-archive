...
void swplayer::camera(int x, int y, int z){
	...
	light_cam->setPosition(vector3df(0,50,0));
...
void swplayer::ch_weapon(int w){
	if(w!=cu_weapon){ 
		...
		if(cu_weapon==5){ we5 =  bullets; we51 = carge; }
...
void swplayer::show_fps(){
	...
	font->draw(gui_buf.c_str(),core::rect<s32>(700,570,300,50),video::SColor(255,255,255,255));
}
